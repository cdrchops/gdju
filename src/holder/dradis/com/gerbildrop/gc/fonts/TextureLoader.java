...
import com.gerbildrop.gc.fonts.Texture;
...
public class TextureLoader {
   ...
   private float textureratio = 1.0f; //for pass on to texture
   /**
    * Create a new texture loader based on the game panel
    *
    * @param gl The GL content in which the textures should be loaded
    */
   public TextureLoader(float whatratio) {
   ...
   private int createTextureID(GL gl)
   {
      ...
      try {
         gl.glGenTextures(1, tmp);
          ...
          System.out.println("Error -- Your system is not capable of running this game.\nPlease make sure your video drivers are current.");
         System.exit(0);
      }
      return tmp.get(0);
   }
   ...
   public Texture getTexture(GL gl, String resourceName) throws IOException {
      ...
      tex = getTexture(gl, resourceName,
            GL.GL_TEXTURE_2D, // target
            GL.GL_RGBA,     // dst pixel format
            GL.GL_LINEAR_MIPMAP_NEAREST, // min filter (unused)
            GL.GL_LINEAR);
   ...
   public Texture getTexture(GL gl, String resourceName,
      ...
      Texture texture = new Texture(gl, target,textureID,textureratio);
      ...
      */
        ...
        new GLU().gluBuild2DMipmaps(target, 4, get2Fold(bufferedImage.getWidth()),
   ...
   public Texture getTexture(GL gl, String resourceName, BufferedImage resourceImage) throws IOException {
      ...
      tex = getTexture(gl, resourceImage,
            GL.GL_TEXTURE_2D, // target
            GL.GL_RGBA,     // dst pixel format
            GL.GL_LINEAR_MIPMAP_NEAREST, // min filter (unused)
            GL.GL_LINEAR);
