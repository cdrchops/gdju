...
//texture definitions used instead of URLS

/**
 * @author vivaldi
 * @version $Id: BasicLoadingGameStateTd.java,v 1.5 2007/04/04 14:29:02 vivaldi Exp $
 * @since Oct 12, 2006
 */
public class BasicLoadingGameStateTd extends GameState implements Loader {
    ...
    public BasicLoadingGameStateTd(TextureDefinition border, TextureDefinition overlay) {
        this(100, border, overlay);
    }

    public BasicLoadingGameStateTd(int steps, TextureDefinition border, TextureDefinition overlay) {
        this.steps = steps;
        current = 0;
        init(border, overlay);
    }
    ...
    protected void init(TextureDefinition border, TextureDefinition overlay) {
        ...
        TextureState ts = border.getTextureState();
...
//        hudNode.setRenderState(loading.getTextureState());
    ...
    public void setProgress(float progress) {
        ...
        if (direction == Direction.LEFT_TO_RIGHT) {
            relCoord = overlay.getTextureWidth() - ((float) percentage / BasicLoadingGameStateTd.MAXIMUM) * overlay.getTextureWidth();
            ...
            relCoord = 0.5f - ((float) percentage / BasicLoadingGameStateTd.MAXIMUM) * 0.5f;
        ...
        if (percentage == 100) {
            BasicLoadingGameStateTd.LoaderFadeOut fader = new BasicLoadingGameStateTd.LoaderFadeOut(2.0f, this);
    ...
    class LoaderFadeOut extends TimedLifeController {
        ...
        private BasicLoadingGameStateTd loading;

        public LoaderFadeOut(float lifeInSeconds, BasicLoadingGameStateTd loading) {
            super(lifeInSeconds);
    ...
    class TransitionFadeIn extends TimedLifeController {
        ...
        private BasicLoadingGameStateTd transition;

        public TransitionFadeIn(float lifeInSeconds, GameState leadIn, BasicLoadingGameStateTd transition) {
            super(lifeInSeconds);
            this.leadIn = leadIn;
