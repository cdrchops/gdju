...
import com.gerbildrop.riverhawk.base.DisplayRenderer;
 ...
 *
 * @author torr
 * @since Nov 20, 2007 - 11:54:42 AM
 */
public abstract class BaseComponent {
    protected boolean rendererIs(DisplayRenderer renderer) {
        return Configuration.DISPLAY_RENDERER == renderer;
    }

    protected DisplayRenderer renderer() {
        return Configuration.DISPLAY_RENDERER;
    }

    protected boolean isJOGL() {
        return Configuration.DISPLAY_RENDERER == DisplayRenderer.JOGL;
    }

    protected boolean isLWJGL() {
        return Configuration.DISPLAY_RENDERER == DisplayRenderer.LWJGL;
    }

    protected GL getGL() {
        return Configuration.getGL();
    }
    ...
    private static boolean configure = true;

    public void configure() {
        m_Scalex = 4;
        m_Scaley = 4;

        parentPhysicalPositionx = 0;
        parentPhysicalPositiony = 0;
    ...
    public void setDimension(final float _width,
                             final float _height) {
        m_PhysicalSizex = _width;
        m_PhysicalSizey = _height;
    }

    public void setTranslation(float x, float y) {
        m_PhysicalPositionx = x;
        m_PhysicalPositiony = y;
    }

    protected void glColor3ub(ByteBuffer bb) {
        byte[] b = bb.array();
        GL11.glColor3ub(b[0], b[1], b[2]);
    }

    protected abstract void drawJOGL(GL gl);
    protected abstract void drawLWJGL();
}
